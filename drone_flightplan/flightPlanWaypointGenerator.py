...
def squaregrid(minx: float, miny: float, maxx: float, maxy: float,
               xspac: float, yspac: float) -> list:
    """
...
if __name__ == "__main__":
    ...
    p.add_argument('-minx', '--min-x', type=float,
                   help = "minimum x coord in EPSG:3857")
    p.add_argument('-maxx', '--max-x', type=float,
                   help = "maximum x coord in EPSG:3857")
    p.add_argument('-miny', '--min-y', type=float,
                   help = "minimum y coord in EPSG:3857")
    p.add_argument('-maxy', '--max-y', type=float,
                   help = "maximum y coord in EPSG:3857")
    ...
    p.add_argument('-ir', '--input_raster', type=str,
                   help = "Digital Elevation Model GeoTIFF file")
    ...
    grid = squaregrid(a.min_x, a.min_y,
                      a.max_x, a.max_y,
                      a.x_spacing, a.y_spacing)

    if a.input_raster:
            try:
                import sampleRasterAtPoints as sr
                grid_with_elevation = sr.sampleRasterFromPointsList(
                    a.input_raster, grid)
